...
sl_lrn_dens <- Lrnr_sl$new(
  learners = list(lrn1_dens, lrn2_dens),
...
```{r manual_vims, eval=FALSE}
# manually fit VIMs over grid using tmle3shift
shift_grid <- seq(-1, 1, 0.5)
shift_grid <- shift_grid[shift_grid != 0]  # don't need to include zero

shifted_vims <- list()
for (this_shift in seq_along(shift_grid)) {
  # initialize a tmle specification
  tmle_spec <- tmle_shift(shift_val = shift_grid[this_shift],
  ...
  tmle3_eif <- as.numeric(sapply(tmle_fit$estimates, `[[`, "IC"))
  tmle3_out <- list(psi = tmle3_psi, se = tmle3_se, eif = tmle3_eif,
                    delta = shift_grid[this_shift])

  ## store parameter estimates across grid delta
  shifted_vims[[this_shift]] <- tmle3_out
}
```
...
To estimate VIMs based on the grid of shift interventions, one may walk through
the procedure to compute TML estimators one function at a time:

```{r fit_tmle_manual, echo=FALSE, eval=FALSE}
# pulls apart the call to tmle3 that follows
...
tmle_params[[1]]$clever_covariates(tmle_task)

fit1 <- fit_tmle3(tmle_task, targeted_likelihood, tmle_params[[1]],
                  updater)
```

Rather than walking through the TML estimation procedure manually, one may use
the `fit_tmle3` convenience function:
